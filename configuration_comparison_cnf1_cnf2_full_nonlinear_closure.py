...
# ============================================================
# Smooth-JUNO closure-test options
# ============================================================

# This script fits the smooth JUNO best-fit spectra rather than the
# fluctuated candidate data. It compares both:
#
#   1. reactor-only model  <-> JUNO reactor_signal
#   2. total model         <-> JUNO reactor_background
#
# This is a model-reproduction or closure test. It is not an
# independent fit to the measured candidate spectrum.
FIT_SMOOTH_JUNO_REACTOR = True
FIT_SMOOTH_JUNO_TOTAL = True

# Relative weights of the two smooth-spectrum terms.
REACTOR_SPECTRUM_WEIGHT = 1.0
TOTAL_SPECTRUM_WEIGHT = 1.0

# Gaussian nuisance-pull penalty:
#
#   1.0 -> standard constrained nuisance fit
#   0.0 -> strict closure test that prioritizes curve overlap
#
# Keep 1.0 for meaningful nuisance constraints. Set to 0.0 only
# when diagnosing whether the model has enough freedom to reproduce
# the black dashed curves.
PULL_PENALTY_WEIGHT = 1.0

# Bin-by-bin closure calibration.
#
# Reactor calibration anchors the zero-pull reactor prediction at
# the published JUNO best-fit oscillation point.
CALIBRATE_REACTOR_TO_JUNO_BEST_FIT = True

# Background calibration aligns the nominal cnf1 background sum to
# the difference between the JUNO total and reactor-only spectra.
CALIBRATE_BACKGROUND_TO_JUNO_BEST_FIT = True
...
# Output paths
FIG_PATH = Path("oscillation_fit_figure2_style_full_nonlinear_closure.png")
RESULTS_PATH = Path("oscillation_fit_cnf1_cnf2_full_nonlinear_closure.npz")
...
# ============================================================
# Smooth JUNO spectra used by the closure fit
# ============================================================

juno_background_on_grid = (
    juno_total_on_grid
    - juno_reactor_on_grid
)

# Tiny interpolation artifacts should not create negative
# background targets.
juno_background_on_grid = np.clip(
    juno_background_on_grid,
    0.0,
    None,
)
...
# ============================================================
# Calibrate the background sum to the smooth JUNO background
# ============================================================

BACKGROUND_BIN_CORRECTION = np.ones_like(
    E_prompt_bins,
    dtype=float,
)

if CALIBRATE_BACKGROUND_TO_JUNO_BEST_FIT:

    reference_background = configuration_cache[
        "cnf1"
    ]["total_background"]

    valid_background_correction = (
        np.isfinite(juno_background_on_grid)
        & np.isfinite(reference_background)
        & (juno_background_on_grid >= 0.0)
        & (reference_background > 0.0)
    )

    BACKGROUND_BIN_CORRECTION[
        valid_background_correction
    ] = (
        juno_background_on_grid[
            valid_background_correction
    ...
    # The correction is only a closure-calibration device.
    # Edge bins outside the useful range should not generate
    # pathological factors.
    BACKGROUND_BIN_CORRECTION = np.clip(
        BACKGROUND_BIN_CORRECTION,
        0.0,
        5.0,
    )
    ...
    for config_name in CONFIGURATIONS:
        ...
        corrected_backgrounds = {}

        for name, spectrum in configuration_cache[
            config_name
        ]["backgrounds"].items():

            corrected_backgrounds[name] = (
                BACKGROUND_BIN_CORRECTION
                * spectrum
            )

        corrected_total = np.zeros_like(
            E_prompt_bins,
            dtype=float,
        )

        for spectrum in corrected_backgrounds.values():
            corrected_total += spectrum

        configuration_cache[
            config_name
        ]["backgrounds"] = corrected_backgrounds

        configuration_cache[
            config_name
        ]["total_background"] = corrected_total

print(
    "Background closure correction in fit range: "
    f"{np.min(BACKGROUND_BIN_CORRECTION[fit_mask]):.4f} to "
    f"{np.max(BACKGROUND_BIN_CORRECTION[fit_mask]):.4f}"
)


# ============================================================
# Calibrate the reactor prediction to the smooth JUNO reactor
# ============================================================

# Establish the absolute normalization with the public
# unoscillated JUNO reactor spectrum.
noosc_weighted_probability = float(
    np.sum(
        reactor_data["w"].to_numpy(
            dtype=float,
        )
    )
)
...
if not np.any(normalization_mask):
    raise ValueError(
        "No common positive bins are available for "
...
# Bin-by-bin oscillated closure calibration evaluated at the
# published JUNO best-fit point.
REACTOR_BIN_CORRECTION = np.ones_like(
    E_prompt_bins,
    dtype=float,
)

if CALIBRATE_REACTOR_TO_JUNO_BEST_FIT:

    closure_survival = compute_weighted_survival(
    ...
    closure_scaled_reactor = (
        C_norm
        * closure_raw_reactor
    )

    valid_reactor_correction = (
        np.isfinite(juno_reactor_on_grid)
        & np.isfinite(closure_scaled_reactor)
        & (juno_reactor_on_grid > 0.0)
        & (closure_scaled_reactor > 0.0)
    )
...
print(
    f"Fixed reactor normalization C_norm = "
    f"{C_norm:.8e}"
)

print(
    "Reactor closure correction in fit range: "
...
def nonlinear_chi2_and_gradient(
    ...
    """
    Full nonlinear objective used in the smooth-JUNO closure fit.

    The objective can include both the smooth reactor-only spectrum
    and the smooth reactor-plus-background spectrum. Nuisance pulls
    remain numerically profiled with L-BFGS-B.
    """

    prediction, components, jacobian_total = (
    ...
    chi2_type = CONFIGURATIONS[
        config_name
    ]["chi2_type"]

    total_chi2 = 0.0
    total_gradient = np.zeros(
        N_NUISANCE,
        dtype=float,
    )

    # --------------------------------------------------------
    # Smooth total-spectrum term
    # --------------------------------------------------------

    if FIT_SMOOTH_JUNO_TOTAL:

        predicted_total_fit = prediction[
            fit_mask
        ]

        target_total_fit = juno_total_on_grid[
            fit_mask
        ]

        jacobian_total_fit = jacobian_total[
            fit_mask,
            :
        ]

        chi2_total, derivative_total = (
            chi2_data_and_derivative(
                observed=target_total_fit,
                predicted=predicted_total_fit,
                chi2_type=chi2_type,
            )
        )
        ...
        total_gradient += (
            TOTAL_SPECTRUM_WEIGHT
            * (
                jacobian_total_fit.T
                @ derivative_total
            )
        )

    # --------------------------------------------------------
    # Smooth reactor-only term
    # --------------------------------------------------------

    if FIT_SMOOTH_JUNO_REACTOR:

        predicted_reactor_fit = components[
            "reactor"
        ][fit_mask]

        target_reactor_fit = juno_reactor_on_grid[
            fit_mask
        ]

        # The reactor Jacobian is the total Jacobian with all
        # background-only columns removed.
        jacobian_reactor = jacobian_total.copy()
        ...
        jacobian_reactor_fit = jacobian_reactor[
            fit_mask,
            :
        ]

        chi2_reactor, derivative_reactor = (
            chi2_data_and_derivative(
                observed=target_reactor_fit,
                predicted=predicted_reactor_fit,
                chi2_type=chi2_type,
            )
        )

        if not np.isfinite(chi2_reactor):
...
def scan_configuration(config_name):
    ...
    # Reactor component from the fully profiled best-fit prediction.
    # This includes the fitted reactor normalization, flux, energy-scale,
    # and energy-resolution pulls.
    best_reactor_with_pulls = best_components["reactor"]
    ...
    return {
        ...
        "best_reactor_with_pulls": best_reactor_with_pulls,
...
np.savez(
    ...
    best_reactor_with_pulls_cnf1=results[
        "cnf1"
    ]["best_reactor_with_pulls"],
    ...
    best_reactor_with_pulls_cnf2=results[
        "cnf2"
    ]["best_reactor_with_pulls"],
    ...
    fit_mode=np.array(
        "full_nonlinear_smooth_juno_closure",
        dtype=str,
    ),
    ...
    fit_target=fit_target_on_grid,
    fit_target_name=np.array(FIT_TARGET_LABEL, dtype=str),
    measured_data=observed_on_grid,
    juno_best_fit_total=juno_total_on_grid,
    juno_best_fit_reactor=juno_reactor_on_grid,
)
...
for config_name, axis in spectrum_axes.items():
    ...
    # Lower histograms: profiled reactor-only component.
    # This includes fitted reactor normalization, flux, scale,
    # and resolution pulls and is therefore the appropriate
    # comparison with the black dashed JUNO reactor spectrum.
    axis.step(
        E_prompt_bins[fit_mask],
        result["best_reactor_with_pulls"][fit_mask],
        where="mid",
        color=color,
        lw=1.8,
        label=config_name,
...
# ============================================================
# Closure diagnostics against the selected fit target
# ============================================================

print()
print("Fit-target closure diagnostics")
print("=" * 78)

for config_name in ["cnf1", "cnf2"]:
    model = results[config_name]["best_prediction"][fit_mask]
    target = fit_target_on_grid[fit_mask]
    ...
    print(
        f"{config_name:6s} | "
        f"RMS residual = {rms_residual:.6f} | "
        f"max |residual| = {max_abs_residual:.6f}"
    )
...
print()
print("=" * 86)
print("SMOOTH-JUNO CLOSURE RESIDUALS")
print("=" * 86)
...
print()
print(
    "Fit target used in chi-square: "
    "smooth JUNO reactor-only and total spectra"
)
print(
    f"Pull penalty weight: {PULL_PENALTY_WEIGHT:.3f}"
)
print(
