...
juno_data_interp = np.interp(x_model, E_juno, juno_data, left=0.0, right=0.0)
...
# reactor_signal is retained only as a plotting reference.
N_obs_reactor = juno_react_interp.copy()

# Use one observation vector in the likelihood.  Do not independently
# fit reactor_signal and reactor_background, because the latter already
# contains the reactor events.
if FIT_TO_MEASURED_DATA:
...
def minimize_profiled_pulls(
    ...
    starts = starts[:PROFILE_MAX_STARTS]
...
if RUN_SOLAR_SCAN:
    ...
    for key, ax in spectra_info:
        ...
        ax.step(
            ...
            linestyle="--",
            lw=1.0,
            label=("JUNO data" if FIT_TO_MEASURED_DATA else "JUNO reactor + backgrounds"),
        )
