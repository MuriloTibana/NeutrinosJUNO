...
# Spectrum used inside the chi-square.
#
#   "juno_best_fit" -> smooth black dashed reactor + background
#                       spectrum from spect-fit.txt
#   "data"          -> measured JUNO candidate spectrum
#
# The default below performs a closure test against the smooth
# black dashed JUNO best-fit spectrum.
FIT_TARGET = "juno_best_fit"
...
# Output paths
FIG_PATH = Path("oscillation_fit_figure2_style_full_nonlinear_juno_target.png")
RESULTS_PATH = Path("oscillation_fit_cnf1_cnf2_full_nonlinear_juno_target.npz")
...
# ============================================================
# Select the spectrum used inside the statistical fit
# ============================================================

if FIT_TARGET == "juno_best_fit":
    # Smooth black dashed reactor + background spectrum.
    fit_target_on_grid = juno_total_on_grid.copy()
    FIT_TARGET_LABEL = "JUNO best-fit reactor + background spectrum"

elif FIT_TARGET == "data":
    # Measured candidate spectrum.
    fit_target_on_grid = observed_on_grid.copy()
    FIT_TARGET_LABEL = "measured JUNO candidate spectrum"

else:
    raise ValueError(
        "FIT_TARGET must be 'juno_best_fit' or 'data'."
    )

print(f"Fit target: {FIT_TARGET_LABEL}")

fit_mask = (
    np.isfinite(fit_target_on_grid)
...
def nonlinear_chi2_and_gradient(
    ...
    observed_fit = fit_target_on_grid[fit_mask]
...
print()
print(f"Fit target used in chi-square: {FIT_TARGET_LABEL}")
print(
