...
# ============================================================
# JUNO-like comparison of analysis configurations cnf 1 and cnf 2
#
# This script:
#   1. Builds a JUNO-like reactor spectrum
#   2. Includes full systematic pulls
#   3. Runs two configurations:
#
#        cnf 1: CNP chi2,    r_BG = 1.00
#        cnf 2: Poisson chi2, r_BG = 1.15
#
#   4. Scans sin^2(theta12) and Delta m21^2
#   5. Profiles over all nuisance pulls at each scan point
#   6. Produces a Fig. 2-like comparison figure:
#        left: contours in solar-parameter space
#        right: best-fit spectra for cnf 1 and cnf 2
#
# Run from project root:
#
#   python configuration_comparison_cnf1_cnf2.py
# ============================================================
...
# Start small while testing. Increase later for smoother contours.
N_SIN2_POINTS = 25
N_DM21_POINTS = 25
...
SIN2_THETA12_RANGE = (0.26, 0.37)
DM21_RANGE = (5.5e-5, 8.8e-5)
...
JUNO_PATH_CANDIDATES = [
    PROJECT_ROOT / "data" / "spect-mine.txt",
    PROJECT_ROOT / "data" / "spect-fit.txt",
    PROJECT_ROOT / "spect-mine.txt",
    PROJECT_ROOT / "spect-fit.txt",
]

DYB_PATH_CANDIDATES = [
    PROJECT_ROOT / "data" / "DYB_unfolded_spectra_tot_U235_Pu239.txt",
    PROJECT_ROOT / "DYB_unfolded_spectra_tot_U235_Pu239.txt",
]
...
NONLINEARITY_CANDIDATES = [
    PROJECT_ROOT / "data" / "positron_nonlinearity.csv",
    PROJECT_ROOT / "positron_nonlinearity.csv",
]
...
Epr_edges = (
    prompt_alpha * np.arange(0.0, 10.0 + bin_width, bin_width)
    + prompt_beta
)
...
JUNO_PATH = first_existing_path(JUNO_PATH_CANDIDATES, "JUNO spectrum file")
print(f"\nUsing JUNO-like file: {JUNO_PATH}")
...
juno_react_bk_interp = np.interp(
    x_model,
    E_juno,
    juno_react_bk,
    left=0.0,
...
DYB_PATH = first_existing_path(DYB_PATH_CANDIDATES, "Daya Bay unfolded spectrum")
print(f"Using Daya Bay file: {DYB_PATH}")
...
def load_or_make_nonlinearity_points() -> tuple[np.ndarray, np.ndarray]:
    try:
        path = first_existing_path(NONLINEARITY_CANDIDATES, "positron nonlinearity CSV")
...
def compute_response_matrix_nl(
    ...
    Evis = E_nu - Delta + m_e
    Epr0 = prompt_alpha * Evis + prompt_beta
...
if SAVE_RESULTS and RUN_SOLAR_SCAN:
    ...
    print(f"\nSaved configuration comparison arrays to:")
    print(f"  {CONFIG_COMPARISON_RESULTS_PATH}")


# ============================================================
# Figure: compare cnf 1 and cnf 2
# ============================================================
...
if RUN_SOLAR_SCAN:
    ...
    gs = fig.add_gridspec(
        2,
        3,
        width_ratios=[3.2, 1.25, 3.0],
        height_ratios=[1.1, 3.0],
        hspace=0.08,
        wspace=0.35,
    )
    ...
    ax_spec2 = fig.add_subplot(gs[1, 2], sharex=ax_spec1)
    ...
    for result in configuration_results:
        ...
        S12_mesh, DM21_mesh = np.meshgrid(
            sin2_grid,
            dm21_grid * 1e5,
        )

        dmax = float(np.max(dchi2_grid))
        contour_levels = [lev for lev in levels_2d if lev < dmax]
        ...
        profile_s12 = np.min(dchi2_grid, axis=0)
        profile_dm21 = np.min(dchi2_grid, axis=1)
    ...
    ax_main.set_xlabel(r"$\sin^2\theta_{12}$", fontsize=14)
    ax_main.set_ylabel(
        r"$\Delta m^2_{21}\,[10^{-5}\,\mathrm{eV}^2]$",
        fontsize=14,
    )

    ax_main.set_xlim(SIN2_THETA12_RANGE)
    ax_main.set_ylim(DM21_RANGE[0] * 1e5, DM21_RANGE[1] * 1e5)
    ...
    ax_top.set_ylabel(r"$\Delta\chi^2$", fontsize=12)
    ax_top.tick_params(labelbottom=False)
    ax_top.grid(True, alpha=0.25)

    ax_right.set_xlabel(r"$\Delta\chi^2$", fontsize=12)
    ax_right.tick_params(labelleft=False)
    ax_right.grid(True, alpha=0.25)

    for level in [1.0, 4.0, 9.0]:
        ax_top.axhline(level, ls=":", lw=1.0, color="gray")
        ax_right.axvline(level, ls=":", lw=1.0, color="gray")

    # --------------------------------------------------------
    # Right-side spectra: cnf 1 and cnf 2
    # --------------------------------------------------------

    spectrum_axes = [ax_spec1, ax_spec2]

    for ax, result in zip(spectrum_axes, configuration_results):
        cfg = result["cfg"]
        scan = result["scan"]
        glob = result["global"]
        ...
        ax.set_ylabel("events per 0.1 MeV", fontsize=10)
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.25)

    ax_spec2.set_xlabel(r"$E_{\rm pr}$ [MeV]", fontsize=12)

    fig.suptitle(
        r"Comparison of analysis configurations: cnf 1 vs cnf 2",
        fontsize=15,
        y=0.98,
    )
    ...
    if SAVE_FIGURES:
        fig.savefig(
            CONFIG_COMPARISON_FIGURE_PATH,
            dpi=250,
            bbox_inches="tight",
        )

        print(f"\nSaved configuration comparison figure to:")
        print(f"  {CONFIG_COMPARISON_FIGURE_PATH}")
